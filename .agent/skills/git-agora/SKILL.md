...
2. **Valide** (se existir JS no projeto): `node --check` nos arquivos alterados e `node --test test/*.test.mjs` (se houver testes).
   Não commite código com erro de sintaxe ou testes falhando.
